<section id="couple" class="couple section">
  <h2>The Couple</h2>
  
  <div class="bride-groom">
    <div class="person">
      <h3>Katie</h3>
      <p>Sometimes when people ask how Dan and I met, I say we met in prison. Really, we met at Libertyville High School, which felt kind of like prison in 1999.</p>
      <p>As a sophomore in Se&ntilde;ora Rogala&#8217;s level 2 Spanish class, I had bangs and carried a cross-body canvas purse with a Pink Floyd patch pinned to it. Dan was a junior with spiky hair, baggy pants, and a quiet coolness detectable from day one. I was into it, not that Dan had any idea.</p>
      <p>Outside of Spanish class, Dan was my favorite guitar player in all of Lake County (in all of anywhere). I organized the local &#8220;band jams,&#8221; and always made sure to give his bands the longest sets and best placement on the bill.</p>
      <p>I also asked him to give me guitar lessons, securing at least one solid hour of quality time each week. But Dan was a horrible teacher. I learned to play about 15 seconds of two songs.</p>
      <p>Our growing friendship continued through college. Dan went to the Savannah College of Art and Design to study Graphic Design, and I went to Ohio University for Journalism (and more Spanish). I interviewed him for magazine stories I was writing. He wrote me 18 haikus when I turned 18.</p>
      <p>After college, we both moved back to Chicago and spent the next year adventuring and learning the ins and outs of the city: late-night bike rides, all-day music festivals, dive bars, and a lot of brunches.</p>
      <p>We were at a holiday party when Dan took me outside, kissed me, and said, &#8220;People are asking if we&#8217;re dating. What should I tell them?&#8221;</p>
    </div>
    <div class="person">
      <h3>Daniel</h3>
      <p>I am not a spontaneous man.
      <p>I didn&#8217;t get my driver&#8217;s license until more than a year after I was legally able to; I don&#8217;t buy anything off Amazon that has less than 20 reviews I can read. And more often than not, I&#8217;ll probably only be halfway through my main course when everyone else at the table is finishing dessert. In fact, the only thing I&#8217;ve ever purposefully done off the cuff is ask Katie Scarlett Brandt to be my wife.</p>
      <p>I had been racking my brain for weeks trying to figure out how I was going to propose to Katie. I&#8217;d bought the ring a month earlier and had been carrying it around in my pocket everywhere we went. A few ideas were floating around (most of them involving elaborate fake alien landings or reenacting Guns n&#8217; Roses&#8217; &#8220;November Rain&#8221; video), but nothing really seemed to stick.</p>
      <p>Then one evening, Katie had just gotten back from another 13-hour drive out to Pine Ridge Indian Reservation in South Dakota, and I was probably sad at myself for not utilizing that weekend alone to convert our apartment into an 10,000-gallon saltwater aquarium, housing a faithful recreation of the shipwreck from "The Little Mermaid" but with our dog Charlie dressed as Flounder and a surprise engagement ring poking out of the sand instead of a fork.*</p>
      <p>So we both wanted sushi. We grabbed a bottle of wine and walked to our favorite place on the corner of our street. She told me about her trip. I lied and told her I&#8217;d had a productive weekend. We talked until the restaurant finally kicked us out, and decided to go for a walk around the neighborhood. After a few blocks, I put my hand in my pocket and felt the ring box.</p>
      <p>Nobody else was around. And I realized that the night we had just had was exactly the type of night I wanted us to have for the rest of our lives. I took a deep breath and got down on one knee to plead my case.</p>
      <p>However, immediately following my heartfelt &#8220;Will You Marry Me?&#8221;, an inebriated gentleman on a mountain bike came barreling down the sidewalk straight towards us, stopping abruptly less than 1 foot away to ask us whether or not we all were &#8220;having a good night&#8221; and &#8220;what&#8217;s up.&#8221;</p>
      <p>Katie was in shock (I like to think from the proposal) and I, not wanting her to answer the Mountain Biker&#8217;s questions before mine, jumped up and informed him that we were having a great night, and not much was up but we would let him know if it turned out that anything was up later. Katie said nothing, and the Mountain Biker went on his way.</p>
      <p>Gathering the courage to propose to the same woman for a second time in the course of 60 seconds, I returned to my position on the sidewalk and brought the ring back out. &#8220;Will You Marry Me?&#8221;</p>
      <p>She said yes. I confirmed. We celebrated.</p>
      <p>I knew immediately that this was the best decision I&#8217;d made in my life, and have started to learn that I can take time when I need it. Eccentricities can turn into strengths as long as you have the right counterpart. I know I&#8217;ve found mine, and that as long as we&#8217;re together we&#8217;ll be alright.</p>
      <p>In the distance, the Mountain Biker ran into a chain link fence on the opposite side of the street, and yelled to let us know he, too, was going to be alright.</p>
      <p class="note">*Disclaimer: Neither of us are really into this movie at all</p>
    </div>
  </div>
  
  <div class="stand-up">
  
    <h2>Stand Up People</h2>
    
    <div class="peeps">
      <div class="person">
        <div class="thumb">
          <img src="/assets/images/pic-wild.jpg" alt="Suzanne Wilder">
        </div>
        <div class="info">
          <h3>Matron of Honor</h3>
          <p><span class="name">Suzanne Wilder</span> make sandwich, I say &#8220;woahh nice!&#8221;. Let&#8217;s drink coffee, it&#8217;s a baby instead.</p>
          <p class="meta"><span class="label">Known for</span> 11 years | <span class="label">First met at</span> band practice</p>
        </div>
      </div>
      <div class="person">
        <div class="thumb">
          <img src="/assets/images/pic-funk.jpg" alt="Ryan Funk">
        </div>
        <div class="info">
          <h3>Best Man</h3>
          <p><span class="name">Ryan Funk</span> is one of my oldest friends, and for good reasons: He always says yes to hanging out before asking what we&#8217;re doing, knows all the Fleetwood Mac b-sides, and is the only person I&#8217;ve ever heard mispronounce their own name when introducing themselves.</p>
          <p class="meta"><span class="label">Known for</span> 16 years | <span class="label">First met at</span> band practice</p>
        </div>
      </div>
      <div class="person">
        <div class="thumb">
          <img src="/assets/images/pic-blane.jpg" alt="Mark Edwin Wetzel">
        </div>
        <div class="info">
          <h3>Officiant</h3>
          <p><span class="name">Mark Edwin Wetzel</span> is Dan&#8217;s musical partner in crime and a best friend to both of us since we all moved to Chicago after college. Mark witnessed our relationship grow from best friends to head-over-heels, and we love having him as the third wheel in our dysfunctional tricycle.</p>
          <p class="meta"><span class="label">Known Dan for</span> 11 years | <span class="label">First met at</span> band practice</p>
          <p class="meta"><span class="label">Known Katie for</span> 8 years | <span class="label">First met at</span> Bill Murray movie</p>
        </div>
      </div>
    </div>
  
  </div>
  
</section>